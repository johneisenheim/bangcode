...
//Rare exceptions to indicate a bug in the _bangwesternduels code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_bangwesternduels = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_bangwesternduels]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.bangwesternduels.module"
