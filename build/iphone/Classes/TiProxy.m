...
//Rare exceptions to indicate a bug in the _bangwd code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_bangwd = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_bangwd]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.bangwd.module"
